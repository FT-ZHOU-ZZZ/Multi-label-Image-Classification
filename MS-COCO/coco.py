...
def download_coco2014(root, phase):
    ...
    cached_file = os.path.join(root, 'annotations_trainval2014.zip')
    if not os.path.exists(cached_file):
...
class COCO2014(data.Dataset):
    ...
    def get_anno(self):
        ...
        print('[dataset] read ' + list_path)
        self.img_list = json.load(open(list_path, 'r'))
        self.cat2idx = json.load(open(os.path.join(self.root, 'data', 'category.json'), 'r'))
        print('[dataset] MS COCO classification set=%s number of classes=%d  number of images=%d' % (
            self.phase, len(self.cat2idx), len(self.img_list)))
